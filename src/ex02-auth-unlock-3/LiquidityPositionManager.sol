...
contract LiquidityPositionManager is ERC6909TokenSupply {
    ...
    function rebalancePosition(
        ...
        delta = abi.decode(
            manager.lock(
                address(this),
                abi.encodeCall(
                    this.handleRebalancePosition,//@audit define callback function
    ...
    ) external returns (BalanceDelta delta) {//@audit called by poolmanager
    ...
    function handleModifyPosition(bytes memory rawData) external returns (BalanceDelta delta) {//@audit called by poolmanager
    ...
    function lockAcquired(address, bytes calldata data) external returns (bytes memory) {//@audit unlockCallback - old style
